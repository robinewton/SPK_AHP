...
$("#formentri").submit(function(e){
	...
	$.ajax({
		type:'post',
		dataType:'json',
		url:"<?=base_url(akses());?>/analisa/kriteria/updatesub",
		data:$(this).serialize(),
		error:function(){
...
$("#prioform").submit(function(e){
	...
	$.ajax({
		type:'post',
		dataType:'json',
		url:"<?=base_url(akses());?>/analisa/kriteria/updatesubprioritas",
		data:$(this).serialize(),
		error:function(){
...
<input type="hidden" name="id_pemilihan_gedung" value="<?=$id_pemilihan_gedung;?>"/>
		...
		for($i=1;$i<=$jumlah;$i++)
		{
			...
			if($noSub==$noUtama)
			{
				...
				{									
					...
					for($x=1;$x<=9;$x++)
					{
						$nilai=ambil_nilai_subkriteria($id_pemilihan_gedung,$id_kriteria,$k2,$xxx);
						$sl='';
						if($nilai==$x)
						{
...
<div class="pull-left">
	<a href="javascript:;" onclick="hitung();" class="btn btn-primary"><i class="fa fa-superscript"></i> Hitung</a> 
	<a href="javascript:;" onclick="showmatrix();" class="btn btn-info"><i class="fa fa-search-plus"></i> Lihat Matriks</a>	
</div>
<div class="pull-right">
	<button type="submit" class="btn btn-success"><i class="fa fa-save"></i> Simpan Kriteria</button>
</div>
<?php
echo form_close();
?>
...
?>
<div class="alert alert-danger">Parameter belum dibuat. Silahkan buat terlebih dahulu <a href="<?=base_url(akses().'/master/kriteria/subkriteria?kriteria='.$id_kriteria);?>">Di sini</a> </div>
<?php
}
?>