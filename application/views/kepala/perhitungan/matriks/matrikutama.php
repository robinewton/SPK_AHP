...
$("#formentri").submit(function(e){
	...
	$.ajax({
		type:'post',
		dataType:'json',
		url:"<?=base_url(akses());?>/analisa/kriteria/updateutama",
		data:$(this).serialize(),
		error:function(){
...
function showsubkriteria()
{
	$.ajax({
			type:'get',
			dataType:'html',
			url:"<?=base_url(akses().'/analisa/kriteria/getsubcontainer');?>",
			data:"id_pemilihan_gedung=<?=$id_pemilihan_gedung;?>",
			error:function(){
		...
		for($i=1;$i<=$jumlah;$i++)
		{
			...
			if($noSub==$noUtama)
			{
				...
				{									
					...
					for($x=1;$x<=9;$x++)
					{
						$nilai=ambil_nilai_kriteria($id_pemilihan_gedung,$k2,$xxx);
						$sl='';
						if($nilai==$x)
						{
...
<div class="pull-left">
	...
	<a href="javascript:;" onclick="showmatrix();" class="btn btn-danger"><i class="glyphicon glyphicon-zoom-out"></i> Lihat Matriks</a>	
	<a href="javascript:;" onclick="showsubkriteria();" class="btn btn-info"><i class="fa fa-search-plus"></i> Lihat Sub Kriteria</a>	
</div>
